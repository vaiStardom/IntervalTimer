...
class IntervalTimerDownloadJSON: NSObject {
...
//            completion(nil, ReachabilityError.notReachable(reason: "Network is unreachable."))
        ...
        let dataTask = session.dataTask(with: request) { (data, response, error) in
            if error == nil {
                    ...
                    case 200: //successful
                        if let data = data {
                            do {
                                ...
                                //throw JsonError.unsucessfulProcessing(reason: "\(error ?? "nil")")
                ...
                //throw UrlError.unsucessfulUrl(reason: "\(error ?? "nil")")
